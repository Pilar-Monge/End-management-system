...
describe('TemporalAutomationService', () => {
  ...
  describe('runDailyResourceCycle', () => {
    ...
    it('processes camps, applying consumption and production', async () => {
      temporalAutomationRepository.findDailyCycleCamps.mockResolvedValue([{ id: 1, minimumDailyRationPerPerson: '2.00', maxPersonCapacity: 10 }]);
      ...
      // Camp Inventory
      temporalAutomationRepository.getOrCreateCampInventory.mockResolvedValue({ currentAmount: '100', minimumAlertAmount: '10', resourceTypeId: 101 });
    ...
    it('handles camp overpopulation and missing staff alerts', async () => {
      temporalAutomationRepository.findDailyCycleCamps.mockResolvedValue([{ id: 1, minimumDailyRationPerPerson: '1.00', maxPersonCapacity: 2 }]);
      ...
      temporalAutomationRepository.findRelevantOccupationsForStaffing.mockResolvedValue([{ id: 1, name: 'Doctor' }]);
  ...
  describe('runHourlyExpeditionStateUpdate', () => {
    ...
    it('updates status and triggers provisioning for newly IN_PROGRESS expedition', async () => {
      ...
      temporalAutomationRepository.findExpeditionsForAutoStateUpdate.mockResolvedValue([
        { id: 1, status: 'PLANNED', plannedDepartureDate: departure, plannedReturnDate: returnDate, extraDaysAvailable: 0, extraDaysUsed: 0, campId: 1 }
      ]);
      
      expeditionParticipantRepository.getActivePersonIdsByExpedition.mockResolvedValue([10]);
      temporalAutomationRepository.findPersonStatusById.mockResolvedValue({ id: 10, currentStatus: 'ACTIVE' });
      expeditionParticipantRepository.getTrackedExpeditionStatusesByPersonId.mockResolvedValue(['IN_PROGRESS']);
      ...
      expect(temporalAutomationRepository.updatePersonStatus).toHaveBeenCalledWith(10, 'ON_EXPEDITION');
    ...
    it('updates status to DELAYED and calculates extra days', async () => {
      ...
      temporalAutomationRepository.findExpeditionsForAutoStateUpdate.mockResolvedValue([
        { id: 1, status: 'IN_PROGRESS', plannedDepartureDate: new Date(0), plannedReturnDate: returnDate, extraDaysAvailable: 5, extraDaysUsed: 0, campId: 1 }
      ]);
    ...
    it('updates status to LOST if past extra days', async () => {
      ...
      temporalAutomationRepository.findExpeditionsForAutoStateUpdate.mockResolvedValue([
        { id: 1, status: 'DELAYED', plannedDepartureDate: new Date(0), plannedReturnDate: returnDate, extraDaysAvailable: 2, extraDaysUsed: 2, campId: 1 }
      ]);
      expeditionParticipantRepository.getActivePersonIdsByExpedition.mockResolvedValue([]);
    ...
    it('updates status to COMPLETED if returned', async () => {
      temporalAutomationRepository.findExpeditionsForAutoStateUpdate.mockResolvedValue([
        { id: 1, status: 'IN_PROGRESS', plannedDepartureDate: new Date(0), plannedReturnDate: new Date(), actualReturnDate: new Date(), extraDaysAvailable: 0, extraDaysUsed: 0, campId: 1 }
      ]);
      expeditionParticipantRepository.getActivePersonIdsByExpedition.mockResolvedValue([]);
