...
describe('ExpeditionParticipantService', () => {
  ...
  describe('createParticipant', () => {
    ...
    it('throws BadRequestException if expeditionId is invalid', async () => {
      await expect(service.createParticipant({ ...validDto, expeditionId: 0 })).rejects.toThrow('expeditionId debe ser un entero positivo');
    });
    ...
    it('throws BadRequestException if expedition is not PLANNED', async () => {
      repository.findExpeditionSummaryById.mockResolvedValue({ ...validExpedition, status: 'IN_PROGRESS' } as never);
      await expect(service.createParticipant(validDto)).rejects.toThrow('Solo las expediciones planificadas pueden recibir nuevos participantes');
    });
    ...
    it('throws BadRequestException if person is INACTIVE', async () => {
      repository.findExpeditionSummaryById.mockResolvedValue(validExpedition as never);
      repository.findPersonById.mockResolvedValue({ ...validPerson, currentStatus: 'INACTIVE' } as never);
      await expect(service.createParticipant(validDto)).rejects.toThrow('Las personas inactivas no pueden asignarse a expediciones');
    });
    ...
    it('throws BadRequestException if person camp mismatches', async () => {
      ...
      await expect(service.createParticipant(validDto)).rejects.toThrow('La persona no pertenece al mismo campamento que la expedicion');
    });
    ...
    it('throws BadRequestException if person has no occupation', async () => {
      ...
      await expect(service.createParticipant(validDto)).rejects.toThrow('La persona debe tener una ocupacion asignada');
    });
    ...
    it('throws BadRequestException if occupation does not participate in expeditions', async () => {
      ...
      repository.findOccupationById.mockResolvedValue({ ...validOccupation, participatesInExpeditions: false } as never);
      await expect(service.createParticipant(validDto)).rejects.toThrow('no esta habilitada para participar en expediciones');
    });
    ...
    it('throws BadRequestException if participant already exists', async () => {
      ...
      await expect(service.createParticipant(validDto)).rejects.toThrow('Este participante de expedicion ya existe');
    });
    ...
    it('creates participant successfully and sends notification', async () => {
      ...
      expect(notificationService.notifyUser).toHaveBeenCalledWith(10, expect.objectContaining({
  ...
  describe('deleteParticipant', () => {
    ...
    it('deletes participant and sends notification', async () => {
      ...
      repository.findExpeditionSummaryById.mockResolvedValue({ id: 1, campId: 1, name: 'Exp 1' } as never);
      repository.findUserByPersonAndCamp.mockResolvedValue({ id: 10 } as never);
