...
import type { ExpeditionResourceConsumedRepository } from './expeditionResourceConsumed.repository';
...
describe('ExpeditionResourceConsumedService', () => {
  ...
  const repository = {
    ...
    delete: jest.fn(),
  } as unknown as jest.Mocked<ExpeditionResourceConsumedRepository>;
  ...
  beforeEach(() => {
    jest.resetAllMocks();
    service = new ExpeditionResourceConsumedService(repository, dataSource, notificationService);
  });
  ...
  describe('createRecord', () => {
    ...
    it('throws if expedition status is invalid', async () => {
      ...
      await expect(service.createRecord(validDto)).rejects.toThrow(
        'No se pueden registrar recursos consumidos',
      );
    });
